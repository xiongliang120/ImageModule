...
/**
 * 加载网络图片，可以配置加载监听，无Options配置项
 * @param any 图片资源
 * @param view 展示的View
 */
fun loadImageNoOptions(any: Any?, view: View?) {
    getStrategy().loadImageNoOptions(any, view)
}
...
fun loadRound(any: Any?, view: View?, roundRadius: Int = 12, roundType: RoundType = RoundType.ALL, placeHolder: Int = 0, errorHolder: Int = 0, listener: OnImageListener? = null
              , isAutoGif: Boolean = false, isEngineRoundedCorners: Boolean = false, roundBorderColor: Int = 0, roundBorderWidth: Int = 0, isBlackWhite: Boolean = false) {
    loadImage(any, view, listener = listener, options = getDefaultBuilder()
            .setCrossFade(false)
            .setAutoGif(isAutoGif)
            .setPlaceHolderResId(placeHolder)
            .setErrorResId(errorHolder)
            .setEngineRoundedCorners(isEngineRoundedCorners)
            .setRoundedCorners(true)
            .setRoundBorderColor(roundBorderColor)
            .setRoundBorderWidth(roundBorderWidth)
